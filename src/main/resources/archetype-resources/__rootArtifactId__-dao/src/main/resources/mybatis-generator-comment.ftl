...
<template>
    ...
    <comment ID="addComment"><![CDATA[
        <!--
  WARNING - 此元素由MyBatis Generator自动生成，请勿修改。
-->
        ]]>
    </comment>
 ...
 * 数据库列备注:<#list introspectedColumn.remarks?split("\n") as remark>
 *   ${remark}
                    </#list>
                </#if>
 * 此字段对应数据库列表 ${introspectedTable.fullyQualifiedTable}.${introspectedColumn.actualColumnName}
 */
            <#else>
/**
 * 该字段对应于数据库表 ${introspectedTable.fullyQualifiedTable}
 */
            </#if>
        ]]>
    </comment>
 ...
 * 数据库表备注:<#list introspectedTable.remarks?split("\n") as remark>
 *   ${remark}
                </#list>
            </#if>
 * 该类对应于数据库表 ${introspectedTable.fullyQualifiedTable}
 */
        ]]>
    </comment>
 ...
 */
    ...
    <comment ID="addEnumComment">
        <![CDATA[
/**
 * 此枚举对应于数据库表 ${introspectedTable.fullyQualifiedTable}
 */
        ]]>
    </comment>
    ...
    <comment ID="addInterfaceComment">
        <![CDATA[
/**
 * 此接口对应于数据库表 ${introspectedTable.fullyQualifiedTable}
 */
        ]]>
    </comment>
    ...
    <comment ID="addGetterComment">
        <![CDATA[
            <#if introspectedColumn??>
/**
 * 此方法返回数据库列的值 ${introspectedTable.fullyQualifiedTable}.${introspectedColumn.actualColumnName}
 * @return the value of ${introspectedTable.fullyQualifiedTable}.${introspectedColumn.actualColumnName}
 */
            <#else>
/**
 * 此方法返回字段的值 ${method.name?replace("get","")?replace("is", "")?uncap_first}
 ...
 */
    ...
    <comment ID="addSetterComment">
        <![CDATA[
            <#if introspectedColumn??>
/**
 * 此方法设置数据库列的值 ${introspectedTable.fullyQualifiedTable}.${introspectedColumn.actualColumnName}
 * @param ${method.parameters[0].name} the value for ${introspectedTable.fullyQualifiedTable}.${introspectedColumn.actualColumnName}
 */
            <#else>
/**
 * 此方法设置字段的值 ${method.name?replace("set","")?uncap_first}
 ...
 */
    ...
    <comment ID="addGeneralMethodComment">
        <![CDATA[
/**
 * 此方法对应于数据库表 ${introspectedTable.fullyQualifiedTable}
 */
        ]]>
    </comment>
</template>